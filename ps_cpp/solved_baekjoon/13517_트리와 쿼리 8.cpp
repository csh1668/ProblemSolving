...
int main() {
    ...
    int tmp = 0;
    comp.reserve(MAXW), decomp.reserve(MAXW);
    sort(tmp_weight + 1, tmp_weight + N + 1);

    for (int i = 1; i <= N; ++i) {
        if (!comp.count(tmp_weight[i])) {
            comp[tmp_weight[i]] = tmp;
            decomp[tmp] = tmp_weight[i];
            tmp++;
        }
    }
    for (int i = 1; i <= N; ++i) {
        weight[i] = comp[weight[i]];
    }
