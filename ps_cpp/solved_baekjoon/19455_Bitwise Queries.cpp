...
struct node
{
	...
	/*
	0110 1100 0100 1110
	mn = 1110 orv = 1110 andv = 0100
	or = 0101
	0111 1101 0101 1111
	mn = 1111 orv = 1111 andv = 0101
...
int search_min(int s, int e, int l, int r, int i = 1) {
	propagate(s, e, i);
	if (e < l || s > r)
		return 1 << 30;
	if (l <= s && e <= r)
		return tree[i].mn;
	const int m = (s + e) >> 1;
	return min(search_min(s, m, l, r, i << 1), search_min(m + 1, e, l, r, i << 1 | 1));
}
...
void and_range(int s, int e, int l, int r, int x, int i = 1) {
	...
	x += (~x & ~tree[i].orv) & ((1 << 30) - 1);
	// x의 모든 0인 비트에 대해 구간 내 모든 비트가 1이라면
	if (l <= s && e <= r && (~x & tree[i].andv) == (~x & (1 << 30) - 1)) {
		// 그냥 빼주면 됨
		lazy[i] -= ((1 << 30) - 1 - x);
		propagate(s, e, i);
		return;
...
int main() {
	...
	for (int i = 0; i < m; ++i) {
		char mode;
		int l, r, x; cin >> mode >> l >> r;
		if (mode == '?') {
			cout << search_min(0, n - 1, l - 1, r - 1) << '\n';
			continue;
		} cin >> x;
		if (mode == '|') {
